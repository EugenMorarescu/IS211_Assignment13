...
def get_results(id):
    ...
    cur = g.db.execute("SELECT quizzes.q_id, results.score, quizzes.q_date, quizzes.q_subject FROM students JOIN results ON students.s_id = results.s_id JOIN quizzes ON results.q_id = quizzes.q_id WHERE students.s_id= ?",[id])
    res = cur.fetchall()
